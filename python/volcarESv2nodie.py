...
while True:
	...
	try:
		...
		#dbms.ping(True)
		cursor=dbms.cursor()
		cursor.execute("""SELECT * FROM prosfiles where pros = 0 order by rand() limit 1""")
		lif=cursor.fetchall()
		...
		for archivocsv in lif:
			...
			print colored('ya no hay archivos que procesar end of for','red')
	...
	print colored('ya no hay archivos que procesar end of while','red')
